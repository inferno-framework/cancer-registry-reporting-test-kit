...
module CancerRegistryReportingTestKit
  module HDEAV100
    class MedicationStatementGroup < Inferno::TestGroup
      title 'Base MedicationStatement Tests'
      short_description 'Verify support for the server capabilities required by the Base MedicationStatement.'
      description %(
  # Background

The US Core Base MedicationStatement sequence verifies that the system under test is
able to provide correct responses for MedicationStatement queries. These queries
must contain resources conforming to the Base MedicationStatement as
...
the [Base MedicationStatement](http://hl7.org/fhir/StructureDefinition/MedicationStatement). Each element is checked against
