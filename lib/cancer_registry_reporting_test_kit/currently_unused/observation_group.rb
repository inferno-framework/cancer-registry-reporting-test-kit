...
module CancerRegistryReportingTestKit
  module HDEAV100
    class ObservationGroup < Inferno::TestGroup
      title 'Observation Base Tests'
      short_description 'Verify support for the server capabilities required by the Base Observation.'
      description %(
  # Background

The US Core Observation Base sequence verifies that the system under test is
able to provide correct responses for Observation queries. These queries
must contain resources conforming to the Base Observation as
...
the [Base Observation](http://hl7.org/fhir/StructureDefinition/Observation). Each element is checked against
