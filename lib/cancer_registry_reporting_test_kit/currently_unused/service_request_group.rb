...
module CancerRegistryReportingTestKit
  module HDEAV100
    class ServiceRequestGroup < Inferno::TestGroup
      title 'Base ServiceRequest Tests'
      short_description 'Verify support for the server capabilities required by the Base ServiceRequest.'
      description %(
  # Background

The US Core Base ServiceRequest sequence verifies that the system under test is
able to provide correct responses for ServiceRequest queries. These queries
must contain resources conforming to the Base ServiceRequest as
...
the [Base ServiceRequest](http://hl7.org/fhir/StructureDefinition/ServiceRequest). Each element is checked against
