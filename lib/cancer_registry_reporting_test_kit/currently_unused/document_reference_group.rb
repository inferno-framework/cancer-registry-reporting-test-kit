...
module CancerRegistryReportingTestKit
  module HDEAV100
    class DocumentReferenceGroup < Inferno::TestGroup
      title 'US Core DocumentReference Tests'
      short_description 'Verify support for the server capabilities required by the US Core DocumentReference Profile.'
      description %(
  # Background

The US Core US Core DocumentReference sequence verifies that the system under test is
...
read succeeds.
      ...
      def self.metadata
        @metadata ||= Generator::GroupMetadata.new(YAML.load_file(
                                                     File.join(__dir__, 'document_reference',
